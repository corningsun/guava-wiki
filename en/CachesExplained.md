...
The canonical way to query a `LoadingCache` is with the method [`get(K)`];
however care must be taken not to call `getUnchecked` on caches
whose `CacheLoader`s declare checked exceptions.
